...
class EventTriggering extends SimulatedAggregateProgram {
  ...
  override def main(): Any = {
    ...
    def detectEvents(): Set[Event] = {
      var value = rep(scala.util.Random.nextInt(100))(_+1) % 100
    ...
    def eventHandlerProcess(e: Event)(args: Unit): POut[Unit] = {
      ...
      node.put(s"${e}_g", g)
      val status = if(g < BUBBLE_RADIUS && t < BUBBLE_DURATION_ROUNDS) Output else External
      POut((), status)
    }
    ...
    def reportEvents(events: Set[Event]): Set[Event] = {
      ...
      sspawn((event: Event) => (_:Unit) => {
        ...
        POut(eventToHandle, if(withinBubble && t < BUBBLE_DURATION_ROUNDS) {
    ...
    val detectedEvents: Set[Event] = branch(eventDetector){ detectEvents() } { Set.empty }
